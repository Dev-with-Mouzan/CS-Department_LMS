...
import {
  BookOpen, Search, Award, CheckCircle2,
...
export default function FinalMarks() {
  ...
  const handleSave = () => {
    ...
    setMessage('')
    setTimeout(() => {
      setMessage('Final year marks saved successfully!')
      setSaving(false)
      ...
      <div className="border border-surface-200 rounded-xl bg-white p-6 space-y-4">
        ...
        {selectedCourse && (
          <div className="max-w-xs">
            <label className="input-label">Full Marks (Final Year)</label>
          ...
          <div className="border border-emerald-200 rounded-xl bg-emerald-50/50 p-5">
            ...
            <div className="space-y-2">
              <div>
                ...
                <input value={bestPaper.student} readOnly className="input-field bg-white/80" placeholder="Auto-detected" />
              </div>
              <div>
                <label className="input-label !text-emerald-700">Marks Obtained</label>
                <input value={bestPaper.marks} readOnly className="input-field bg-white/80" placeholder="—" />
              </div>
            </div>
          </div>
          ...
          <div className="border border-red-200 rounded-xl bg-red-50/50 p-5">
            ...
            <div className="space-y-2">
              <div>
                ...
                <input value={worstPaper.student} readOnly className="input-field bg-white/80" placeholder="Auto-detected" />
              </div>
              <div>
                <label className="input-label !text-red-700">Marks Obtained</label>
                <input value={worstPaper.marks} readOnly className="input-field bg-white/80" placeholder="—" />
              </div>
            </div>
          </div>
        </div>
      ...
      {selectedCourse && students.length > 0 && (
        <div className="flex justify-end">
          <Button onClick={handleSave} disabled={saving}>
            ...
            {saving ? 'Saving...' : 'Save Final Year Marks'}
          </Button>
        </div>
      )}
    </div>
